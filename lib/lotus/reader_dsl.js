...
[ 1, 2, 4 ].forEach(function (bytes) {
  ...
  proto['u' + bits + 'be'] = function (prop, decode) {
    var fn = buildBytes(bytes, 'U', 'BE', prop, decode);
    this.stack.push(fn);
    return this;
  };

  proto['u' + bits + 'le'] = function () {
    var fn = buildBytes(bytes, 'U', 'LE', prop, decode);
    this.stack.push(fn);
    return this;
  };

  proto['s' + bits + 'be'] = function () {
    var fn = buildBytes(bytes, 'S', 'BE', prop, decode);
    this.stack.push(fn);
    return this;
  };

  proto['s' + bits + 'le'] = function () {
    var fn = buildBytes(bytes, 'S', 'LE', prop, decode);
    this.stack.push(fn);
    return this;
  };
});
...
function buildBytes (bytes, signed, endian, as, decode) {
  return function (msg, bufs, next) {
    waitFor(bytes, bufs, function (err, res) {
      if (err) return next(err);
      var int = util['decode' + endian + signed](res);
      msg[as] = decode && 'function' === typeof decode
        ? decode(int)
        : int;
      next();
